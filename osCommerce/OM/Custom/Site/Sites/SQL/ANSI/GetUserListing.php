...
class GetUserListing
{
    public static function execute(array $params)
    {
        ...
        $sql = 'select s.id, s.public_id, s.title, s.url, s.status, c.countries_name as country_name, c.countries_iso_code_2 as country_code, cat.categories_name as category_name, parent_cat.categories_name as parent_category_name, count(l.id) as total_likes from :table_website_live_shops s left join :table_website_live_shops_likes l on (s.id = l.live_shop_id), :table_countries c, :table_website_live_shops_categories cat left join :table_website_live_shops_categories parent_cat on (cat.parent_id = parent_cat.categories_id) where s.user_id = :user_id';
