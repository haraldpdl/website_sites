...
class GetCountries
{
    public static function execute(array $params)
    {
        ...
        if ($params['category_id'] === -1) {
            $Qcountries = $OSCOM_PDO->prepare('select countries_id as id, countries_name as title, countries_iso_code_2 as code from :table_countries order by countries_name');
            $Qcountries->setCache('sites-countries-all');
            $Qcountries->execute();
        } else {
            ...
            $Qcountries->setCache('sites-countries-active');
            $Qcountries->execute();
        }
