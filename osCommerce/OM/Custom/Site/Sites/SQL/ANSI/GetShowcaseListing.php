...
class GetShowcaseListing
{
    public static function execute(array $params)
    {
        ...
        $sql = '
            select
                s.id, s.public_id, s.title, s.url,
                c.countries_name as country_name, c.countries_iso_code_2 as country_code,
            ...
            from
                ...
                left join :table_website_live_shops_likes l on (s.id = l.live_shop_id)
                inner join :table_website_partner p on (sc.partner_id = p.id)
                inner join :table_countries c on (s.country_id = c.countries_id)
                inner join :table_website_live_shops_categories cat on (s.category_id = cat.categories_id)
                left join :table_website_live_shops_categories parent_cat on (cat.parent_id = parent_cat.categories_id)
            where
                p.code = :partner_code and
                s.status = 3
            group by
                s.id
            order by
